...
- **Enhanced RLE8 デコーダー**: sff2_decode.py ベースの高精度RLE8圧縮スプライト解析
...
## Enhanced RLE8 デコーダー について

バージョン 2.1.0 より、sff2_decode.py ベースの強化されたRLE8デコーダーを統合しました。

### 特徴

1. **高精度デコード**: Elecbyte公式仕様に準拠したRLE8デコーダー
2. **パフォーマンス最適化**: NumPyベースの効率的な処理
3. **自動フォールバック**: 従来のデコーダーとの互換性維持
4. **キャッシュ機能**: 同一ファイルの高速再読み込み

### 対応フォーマット

- **SFFv2 RLE8 (fmt=2, coldepth=8)**: Enhanced SFF2デコーダーを優先使用
- **従来フォーマット**: 既存のデコーダーを使用
- **PNG (fmt=10)**: PNG形式スプライト対応

### 技術詳細

Enhanced RLE8デコーダーは以下の手順で動作します：

1. **fmt=2, coldepth=8 の検出**: RLE8対象スプライトを自動識別
2. **Enhanced SFF2 Reader の作成**: sff2_decode.py ベースのリーダー初期化
3. **RLE8デコード**: Elecbyte仕様に準拠したデコード処理
4. **アルファ値修正**: SFFv2パレットの不適切なアルファ値を自動修正
   - Index 0 (背景): 透明 (alpha=0)
   - Index 1-255: 不透明 (alpha=255)
5. **RGBA変換**: GUI表示用の32ビットRGBA形式に変換
6. **フォールバック**: 失敗時は従来デコーダーを使用

### アルファ値修正について

多くのSFFv2ファイルでは、パレットのアルファ値が0や1などの不適切な値に設定されており、
そのまま使用するとスプライトが透明すぎて見えなくなる問題があります。

Enhanced RLE8デコーダーでは以下の修正を自動的に適用します：

- **背景色 (Index 0)**: 完全透明 (alpha=0)
- **その他の色**: 完全不透明 (alpha=255)
- **過度に透明なスプライト**: RGB値が0でないピクセルを自動的に不透明化

この修正により、本来の色彩でスプライトが正常に表示されます。
